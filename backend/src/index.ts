...
const start = async () => {
    try {
        ...
        io.on('connection', (socket) => {
            ...
            socket.on('device:handshake', async (data: { provisioningKey: string, hardwareHash?: string, type?: string }) => {
                try {
            ...
            socket.on('web:handshake', (data: { userId: string, publicKey: string }) => {
            ...
            socket.on('relay:tunnel', (data: { targetId: string, payload: any }) => {
            ...
            socket.on('device:approve', async (data: { deviceId: string, passwordHash: string }) => {
                const userId = socketRegistry.get(socket.id);
                if (!userId) return;
            ...
            // 5. Command Execution
            socket.on('command:exec', async (data: { targetDeviceId: string, cmd: string, params: any }) => {
                const userId = socketRegistry.get(socket.id);
                if (!userId) return;
